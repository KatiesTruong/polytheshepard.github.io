...
function saveBookmark(e) {
    ...
    // If local storage with bookmark is null we initialise it into an array,
    // then create an empty array called bookmarks and push the bookmark objects
    // into the empry array
...
function fetchBookmarks() {
    ...
    for (var i = 0; i < bookmarks.length; i++) {
        ...
        /// Call the bookmarks array and push bookmark into that array and
        /// append this to div with +=
        bookmarkResults.innerHTML += '<div class="bookmark-group">' +
            '<h3>' + name + '</h3>' +
            '<button name="button"><a target="_blank" href="' + url +
            '">Visit</a></button>' +
            '<button onclick="deleteBookmark(\'' + url +
            '\')" name="button"><a href="#">Delete</a></button>' +
            '</div>';
    }
}
